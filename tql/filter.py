...
FILTERS = {
    ...
    'str': (lambda s: str(s), 0, "<col>|str", "any", "str", "Convert to string."),

    # Human
    'humanize': (lambda s, u: humanize(to_num(s), u, show_value=False), 1,
                 "<col>|humanize:<unit>", "num", "str", "Format number to human string."),
    ...
    'ljust': (lambda s, w: s.ljust(to_int(w)), 1, "<col>|ljust:<width>", "str", "str",
              "Left justify string in <width> spaces."),
    'rjust': (lambda s, w: s.rjust(to_int(w)), 1, "<col>|rjust:<width>", "str", "str",
              "Right justify string in <width> spaces."),
    'center': (lambda s, w: s.center(to_int(w), ' '), 1, "<col>|center:<width>", "str", "str",
               "Center string in <width> spaces."),
    'swapcase': (lambda s: s.swapcase(s), 0, "<col>|swapcase", "str", "str", "Swap string case."),
    'replace': (lambda s, x, y: s.replace(x, y), 2, "<col>|replace:<from>,<to>", "str", "str",
                "Replace sub-string <from> with <to>."),
    'title': (lambda s: s.title(), 0, "<col>|title", "str", "str", "Convert string to title case."),
    'zfill': (lambda s, w: s.zfill(to_int(w)), 1, "<col>|zfill:<width>", "str", "str",
    ...
    'lstrip': (lambda s, c: s.lstrip(c), 1, "<col>|lstrip:<chars>", "str", "str",
               "Strip <chars> from the left end of the string."),
    'rstrip': (lambda s, c: s.rstrip(c), 1, "<col>|rstrip:<chars>", "str", "str",
    ...
    # Data formatting
    'format': (lambda x, fmt: format(to_num(x), fmt), 1, "<col>|format:<format>", "str", "str",
    ...
    'datetime_tz': (lambda s, tz: pendulum.parse(s, tz=tz), 1, "<col>|datetime_tz:<tz>", "str", "datetime",
                    "Prase a datetime string with the specified <tz> timezone."),
    'tz': (lambda dt, tz: pendulum.timezone(tz).convert(dt), 1, "<col>|tz:<tz>", "datetime", "datetime",
           "Convert a datetime to a new <tz> timezone."),
    'iso8601': (lambda dt: dt.to_iso8601_string(), 0, "<col>|iso8601", "datetime", "str",
                "Convert a datetime to an ISO8601 string representation."),
    'utc': (lambda dt: pendulum.timezone('UTC').convert(dt), 0, "<col>|utc", "datetime", "datetime",
            "Convert a datetime to UTC."),
    'strftime': (lambda dt, fmt: dt.strftime(fmt), 1, "<col>|strftime:<format>", "datetime", "str",
